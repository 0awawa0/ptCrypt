...
class Curve:
	...
	class Point:
		...
		def __add__(self, other):
			...
			if self.x == 'O': return other
			if other.x == 'O': return self

			if self == -other: return self.curve.point('O', 'O')
			...
			if self != other:
				...
				if self.curve.p:
					...
					if r != 1:
						print(f"[WARNING] Failed to add {self} and {other} for curve {self.curve}. {low} does not have inverse modulo {self.curve.p}. GCD is {r}")
						return r
		...
		def __mul__(self, number):
			...
			while n > 0:
				if n % 2:
					R = R + Q
					if type(R) is int: return R
				Q = Q + Q
				if type(Q) is int: return Q
				n = n >> 1
			return R

...
class DSA:
    ...
    def generateProbablePrimes(N: int, L: int, seedLength: int, hashFunction=hashlib.sha256):
        ...
        if seedLength < N:
            DSA.Params.ProbablePrimesGenerationResult(False, None, None)
        ...
        while 1:
            ...
            for counter in range(0, 4 * L - 1):
                W = 0
                for j in range(0, n):
                    hashPayload = mathbase.intToBytes(
                        (domainParameterSeed + offset + j) % twoPowSeedLength)
                    v = mathbase.bytesToInt(hashFunction(hashPayload).digest())
                    W += v * pow(twoPowOutLength, j)

                hashPayload = mathbase.intToBytes(
                    (domainParameterSeed + offset + n) % twoPowSeedLength)
                ...
                if p >= twoPowLMin1:
                    if mathbase.isPrime(p, 10):
                        ...
                        verifyParams = DSA.Params.ProbablePrimesGenerationResult.ProbablePrimesVerifyParams(
                            domainParameterSeed, counter)
    ...
    def verifyProbablePrimesGenerationResult(result, hashFunction=hashlib.sha256) -> bool:
        ...
        U = mathbase.bytesToInt(hashFunction(
